    ...
    int srcw, int srch, int src_stride, int outw, int outh, int outcstep, int inferw, int inferh)
{
    ...
    constants[0].i = srcw;
    constants[1].i = srch;
    constants[2].i = src_stride;
    constants[3].i = outw;
    constants[4].i = outh;
    constants[5].i = outcstep;
    constants[6].i = srcw;
    constants[7].i = srch;
    constants[8].i = inferw;
    constants[9].i = inferh;
    return constants;
}

static std::vector<ncnn::vk_constant_type> make_rife_postproc_constants(
    int srcw, int srch, int srccstep, int outw, int outh, int out_stride, int inferw, int inferh)
{
    std::vector<ncnn::vk_constant_type> constants(10);
    constants[0].i = srcw;
    constants[1].i = srch;
    constants[2].i = srccstep;
    constants[3].i = outw;
    constants[4].i = outh;
    constants[5].i = out_stride;
    constants[6].i = outw;
    constants[7].i = outh;
    constants[8].i = inferw;
    constants[9].i = inferh;
...
RIFE::RIFE(int gpuid, bool _tta_mode, bool _tta_temporal_mode, bool _uhd_mode, int _num_threads, bool _rife_v2, bool _rife_v4)
    : RIFE(gpuid == -1 ? 0 : ncnn::get_gpu_device(gpuid), _tta_mode, _tta_temporal_mode, _uhd_mode, _num_threads, _rife_v2, _rife_v4)
{
}

RIFE::RIFE(ncnn::VulkanDevice* _vkdev, bool _tta_mode, bool _tta_temporal_mode, bool _uhd_mode, int _num_threads, bool _rife_v2, bool _rife_v4)
{
    vkdev = _vkdev;
...
//     fprintf(stderr, "%d x %d\n", w, h);
    ...
    if (tta_mode)
    {
        ...
        ncnn::VkMat in1_gpu_padded[8];
        {
            ...
            std::vector<ncnn::vk_constant_type> constants = make_rife_preproc_constants(
                in0_gpu.w, in0_gpu.h, in0_gpu.cstep,
                in0_gpu_padded[0].w, in0_gpu_padded[0].h, in0_gpu_padded[0].cstep,
                in0_gpu.w, in0_gpu.h);
        ...
        }
        {
            ...
            std::vector<ncnn::vk_constant_type> constants = make_rife_preproc_constants(
                in1_gpu.w, in1_gpu.h, in1_gpu.cstep,
                in1_gpu_padded[0].w, in1_gpu_padded[0].h, in1_gpu_padded[0].cstep,
                in1_gpu.w, in1_gpu.h);
        ...
        // postproc
        {
            ...
            std::vector<ncnn::vk_constant_type> constants = make_rife_postproc_constants(
                out_gpu_padded[0].w, out_gpu_padded[0].h, out_gpu_padded[0].cstep,
                out_gpu.w, out_gpu.h, out_gpu.cstep,
                out_gpu.w, out_gpu.h);
    ...
    else
    {
        ...
        // postproc
        {
            ...
            std::vector<ncnn::vk_constant_type> constants = make_rife_postproc_constants(
                out_gpu_padded.w, out_gpu_padded.h, out_gpu_padded.cstep,
                out_gpu.w, out_gpu.h, out_gpu.cstep,
                out_gpu.w, out_gpu.h);
...
int RIFE::process_v4_gpu(const ncnn::VkMat& in0image, const ncnn::VkMat& in1image, int width, int height, int inference_width, int inference_height, float timestep, ncnn::VkMat& outimage) const
{
    ...
    if (width <= 0 || height <= 0 || inference_width <= 0 || inference_height <= 0 || in0image.empty() || in1image.empty())
        return -3;
    ...
    inference_width = inference_width < width ? inference_width : width;
    inference_height = inference_height < height ? inference_height : height;

    const int w_padded = (inference_width + 31) / 32 * 32;
    const int h_padded = (inference_height + 31) / 32 * 32;
    const size_t in_out_tile_elemsize = opt.use_fp16_storage ? 2u : 4u;

    if (outimage.empty())
        outimage.create(width, height, (size_t)3u, 1, blob_vkallocator);
...

    {
        ...
        std::vector<ncnn::vk_constant_type> constants = make_rife_postproc_constants(
            out_gpu_padded.w, out_gpu_padded.h, out_gpu_padded.cstep,
            width, height, static_cast<int>(outimage.elemsize),
            inference_width, inference_height);

        cmd.record_pipeline(rife_postproc, bindings, constants, outimage);
    }
